...
def scrape(start_page: Optional[int] = None, end_page: Optional[int] = None, out_csv: Optional[str] = None, debug: bool = False, stagnant_limit: int = DEFAULT_STAGNANT_LIMIT, max_dom_nodes: int = DOM_NODE_CAP_DEFAULT, allow_no_price: bool = False):
    ...
    # Session control (env set by CLI):
    fresh_session = os.environ.get("WM_FRESH_SESSION", "0") in ("1","true","True")
    clear_session = os.environ.get("WM_CLEAR_SESSION", "0") in ("1","true","True")
    state_dir = os.path.join(DATA_DIR, ".pw_state")
    if clear_session and os.path.isdir(state_dir):
        import shutil
        try:
            shutil.rmtree(state_dir)
            log(f"[session] Cleared persisted state at {state_dir}")
        except Exception as e:
            log(f"[session-warn] Failed to clear state: {e}")

    with sync_playwright() as pw:
        if fresh_session:
            log("[session] Using FRESH (non-persistent) browser context")
            browser = pw.chromium.launch(headless=HEADLESS)
            ctx = browser.new_context(user_agent=USER_AGENT) if USER_AGENT else browser.new_context()
        else:
            log("[session] Using PERSISTENT browser context")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fresh-session", action="store_true", help="Use a temporary (non-persistent) browser context for this run only")
    parser.add_argument("--clear-session", action="store_true", help="Delete persisted playwright state directory before starting (implies persistent context)")
    ...
    # Session handling
    os.environ["WM_FRESH_SESSION"] = "1" if args.fresh_session else os.environ.get("WM_FRESH_SESSION", "0")
    os.environ["WM_CLEAR_SESSION"] = "1" if args.clear_session else os.environ.get("WM_CLEAR_SESSION", "0")
